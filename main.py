...
from config import (
    api_id, api_hash, bot_token, auth_users, 
    FORCE_SUB_CHANNEL, FORCE_SUB_CHANNEL_LINK, 
    CHANNEL_JOIN_PHOTO, CHANNEL_JOIN_TEXT, 
    AUTH_MODE, MAX_FILES_PER_USER, 
    LOG_CHANNEL
)
...
try:
    ...
    db = client.bot_db
    quota_collection = db.user_quota
...
def run_flask():
    app.run(host="0.0.0.0", port=8080)  # Changed port to 8080 for Koyeb compatibility
    

image_list = [
"https://i.imghippo.com/files/jela9414yEM.jpg",
"https://i.imghippo.com/files/zlP1681tss.jpg",
"https://i.imghippo.com/files/KSH5602grQ.jpg",
"https://i.imghippo.com/files/gLy7080g.webp",
"https://i.imghippo.com/files/hviq4677ng.png",
]
print(4321)

# Function to format channel link
def format_channel_link(link):
    if not link:
        return None
    # Remove any @ symbol
    link = link.replace('@', '')
    ...
    if link.startswith('https://t.me/'):
        return link
    # If it's just a username/channel name, add the prefix
    return f"https://t.me/{link}"

# Function to check if user is subscribed to the channel
async def check_subscription(user_id: int) -> bool:
    try:
        # Skip subscription check for bot owner and authorized users
...
# Function to send file to log channel with better error handling
async def send_to_log_channel(file_path: str, caption: str, user_id: int) -> bool:
    try:
        if not LOG_CHANNEL:
            logging.error("LOG_CHANNEL is not configured in config.py")
            return False
            
        # Convert LOG_CHANNEL to integer if it's a string
        try:
            log_channel_id = int(LOG_CHANNEL)
        except ValueError:
            logging.error(f"Invalid LOG_CHANNEL format: {LOG_CHANNEL}")
            return False
        ...
        try:
            ...
            if not chat:
                logging.error(f"Could not find log channel: {log_channel_id}")
                return False
        except Exception as e:
            logging.error(f"Error accessing log channel: {e}")
            return False
            
        user = await bot.get_users(user_id)
        username = user.username or "No Username"
        log_text = (
            f"**📥 New File Extracted**\n\n"
        ...
        try:
            ...
            logging.info(f"Successfully sent file to log channel for user {user_id}")
            return True
        except Exception as e:
            logging.error(f"Error sending file to log channel: {e}")
            return False
    except Exception as e:
        logging.error(f"Unexpected error in send_to_log_channel: {e}")
        return False
...
async def start(bot, message):
        ...
        try:
            ...
            try:
                await message.reply_photo(
                    photo=CHANNEL_JOIN_PHOTO,
                    caption=CHANNEL_JOIN_TEXT,
                    reply_markup=reply_markup
                )
            except Exception as e:
                logging.error(f"Error sending photo: {e}")
                await message.reply_text(
                    text=CHANNEL_JOIN_TEXT,
                    reply_markup=reply_markup
                )
        except Exception as e:
    ...
    # If user is subscribed, show the main menu
    try:
        ...
        try:
            ...
            logging.error(f"Error sending menu photo: {e}")
            await message.reply_text(
...
async def check_subscription_callback(bot, callback_query):
    ...
    try:
        ...
        if is_subscribed:
            ...
            # Show join channel message again
            keyboard = [
                [InlineKeyboardButton("📢 Join Channel", url=FORCE_SUB_CHANNEL_LINK)],
                [InlineKeyboardButton("🔄 Check Subscription", callback_data="check_sub")]
            ]
    ...
    for attempt in range(max_retries):
        try:
            if isinstance(session, aiohttp.ClientSession):
                ...
                logging.error("Invalid session type. Expected aiohttp.ClientSession")
                return None
...
# Helper function to check subscription and auth
async def check_access(user_id: int, message) -> bool:
    try:
        ...
        # Then check auth if enabled
        if AUTH_MODE and user_id not in auth_users:
            auth_user = auth_users[0]
            user = await bot.get_users(auth_user)
            owner_username = "@" + user.username
            await message.edit_text(f"**You Are Not Authorized To Use This Bot\nContact - {owner_username}**")
            return False

        return True
    except Exception as e:
        logging.error(f"Error in check_access: {e}")
        return False
...
async def pwwp_callback(bot, callback_query):
    ...
    if not await check_access(user_id, callback_query.message):
        return
    ...
    async with aiohttp.ClientSession(connector=CONNECTOR, loop=loop) as session:
        try:
            ...
            if courses:
                ...
                await editable.edit(f"**Extracting course : {selected_batch_name} ...**\n\n💡 Please be patient as we process your request on our free server. We appreciate your understanding and gentle usage.")
...
async def cpwp_callback(bot, callback_query):
    ...
    try:
        ...
        logging.error(f"Error in cpwp_callback: {e}")
...
async def process_cpwp(bot: Client, m: Message, user_id: int):
    try:
        # Check quota at the start
        if not await check_auth(user_id):
            remaining = await get_remaining_quota(user_id)
            if remaining <= 0:
                owner = await bot.get_users(auth_users[0])
                owner_username = owner.username or "No Username"
                await m.reply(
                    f"**⚠️ You have reached your daily limit of {MAX_FILES_PER_USER} files.\n\n"
                    f"**For unlimited access, contact:**\n"
                                        ...
                                        if response.status == 200:
                                            ...
                                            if course_content:
                                                ...
                                                try:
                                                    ...
                                                    # Send file to log channel
                                                    log_success = await send_to_log_channel(file, caption, user_id)
                                                    if not log_success:
                                                        logging.error("Failed to send file to log channel")
                                                        
                                                except Exception as e:
                                                    logging.error(f"Error in process_cpwp: {e}")
        ...
        # After successful file extraction, update quota
        if not await check_auth(user_id):
            await update_quota(user_id)
            remaining = await get_remaining_quota(user_id)
            await show_quota_message(m, user_id, remaining)
...
async def appxwp_callback(bot, callback_query):
    ...
    try:
        ...
        logging.error(f"Error in appxwp_callback: {e}")
        await callback_query.message.reply(f"**Error: {e}**")
    finally:
        loop.close()
...
async def process_appxwp(bot: Client, m: Message, user_id: int):
    try:
        ...
        logging.error(f"Error in process_appxwp: {e}")
        await m.reply(f"**Error: {e}**")


# Add auth mode toggle commands
@bot.on_message(filters.command(["on"]) & filters.user(auth_users))
async def enable_auth(bot, message):
    global AUTH_MODE
    AUTH_MODE = True
    await message.reply("**Auth mode enabled. Only authorized users can use the bot.**")

@bot.on_message(filters.command(["off"]) & filters.user(auth_users))
async def disable_auth(bot, message):
    global AUTH_MODE
    AUTH_MODE = False
    await message.reply("**Auth mode disabled. Any subscribed user can use the bot.**")

# Helper function to check auth
async def check_auth(user_id: int) -> bool:
    try:
        auth_user = await auth_collection.find_one({"user_id": user_id})
        return auth_user is not None
    except Exception as e:
        logging.error(f"Error checking auth status: {e}")
        return False

# Helper function to get remaining quota with improved error handling
async def get_remaining_quota(user_id: int) -> int:
    try:
        # Get user's quota from MongoDB
        user_quota = await quota_collection.find_one({"user_id": user_id})
        if not user_quota:
        ...
        last_reset = user_quota.get("last_reset", datetime.now())
        if datetime.now() - last_reset > timedelta(hours=24):
            # Reset quota
            await quota_collection.update_one(
                {"user_id": user_id},
                {"$set": {"files_extracted": 0, "last_reset": datetime.now()}}
            )
            return MAX_FILES_PER_USER
        
        files_extracted = user_quota.get("files_extracted", 0)
        return max(0, MAX_FILES_PER_USER - files_extracted)
    except Exception as e:
        logging.error(f"Error getting remaining quota: {e}")
        # In case of database error, allow the operation to proceed
        return MAX_FILES_PER_USER

# Helper function to update quota with improved error handling
async def update_quota(user_id: int) -> bool:
    try:
        result = await quota_collection.update_one(
            {"user_id": user_id},
            {"$inc": {"files_extracted": 1}},
            upsert=True
        )
        return result.modified_count > 0 or result.upserted_id is not None
    except Exception as e:
        logging.error(f"Error updating quota: {e}")
        return False

# Helper function to show quota message
async def show_quota_message(message: Message, user_id: int, remaining: int):
    try:
        owner = await bot.get_users(auth_users[0])
        owner_username = owner.username or "No Username"
        
        quota_message = (
            f"**📊 Your Remaining Quota:**\n"
            f"📝 **Files Left:** {remaining}/{MAX_FILES_PER_USER}\n\n"
            f"**For unlimited access, contact:**\n"
            f"👤 **Admin:** @{owner_username}\n\n"
            f"**Your ID:** `{user_id}`"
        )
        ...
        logging.error(f"Error showing quota message: {e}")

# Helper function to check and send file with quota
async def check_and_send_file(user_id: int, file_path: str, caption: str, message: Message) -> bool:
    try:
        ...
        if not await check_auth(user_id):
            ...
            try:
                ...
                # Update quota and show remaining
                await update_quota(user_id)
                remaining = await get_remaining_quota(user_id)
                await show_quota_message(message, user_id, remaining)
                return True
            except Exception as e:
                logging.error(f"Error sending file: {e}")
                return False
        else:
            # Authorized user - just send file without quota message
            try:
                with open(file_path, 'rb') as f:
                    await message.reply_document(
                        document=f,
        ...
        logging.error(f"Error in check_and_send_file: {e}")
        return False

# Update auth commands to use MongoDB
@bot.on_message(filters.command(["auth"]) & filters.user(auth_users[0]))
async def add_auth_user(bot, message):
    try:
        user_id = int(message.text.split()[1])
        if not await check_auth(user_id):
            await auth_collection.insert_one({"user_id": user_id})
            await message.reply(f"**User {user_id} has been authorized to use the bot without limits.**")
        else:
            await message.reply("**User is already authorized.**")
    except (IndexError, ValueError):
        await message.reply("**Please provide a valid user ID.**")

@bot.on_message(filters.command(["rem"]) & filters.user(auth_users[0]))
async def remove_auth_user(bot, message):
    try:
        user_id = int(message.text.split()[1])
        if await check_auth(user_id) and user_id != auth_users[0]:  # Prevent removing bot owner
            ...
            await message.reply("**User is not in the authorized list or is the bot owner.**")
    except (IndexError, ValueError):
        await message.reply("**Please provide a valid user ID.**")

@bot.on_message(filters.command(["users"]) & filters.user(auth_users[0]))
async def list_auth_users(bot, message):
    user_list = "**Authorized Users:**\n\n"
    async for user in auth_collection.find():
        user_id = user["user_id"]
        try:
            user_info = await bot.get_users(user_id)
            username = user_info.username or "No Username"
            user_list += f"**Name:** {user_info.first_name}\n**Username:** @{username}\n**ID:** `{user_id}`\n\n"
        except:
            user_list += f"**ID:** `{user_id}`\n\n"
    await message.reply(user_list)

@bot.on_message(filters.command(["stats"]) & filters.user(auth_users[0]))
async def show_stats(bot, message):
    try:
        # Get all users from quota collection
        all_users = []
        async for user in quota_collection.find():
            all_users.append(user)
            
        # Get authorized users
        auth_users_list = []
        async for user in auth_collection.find():
            auth_users_list.append(user["user_id"])
            
        # Sort users by files extracted
        all_users.sort(key=lambda x: x.get("files_extracted", 0), reverse=True)
        
        # Prepare message
        message_text = "**📊 Bot Usage Statistics**\n\n"
        
        # Regular users section
        message_text += "**👥 Regular Users:**\n"
        for user in all_users[:10]:  # Show top 10 users
            user_id = user["user_id"]
            if user_id not in auth_users_list and user_id != auth_users[0]:
                try:
                    user_info = await bot.get_users(user_id)
                    username = user_info.username or "No Username"
                    files_extracted = user.get("files_extracted", 0)
                    message_text += f"• **{user_info.first_name}** (@{username})\n"
                    message_text += f"  └ Files Extracted: {files_extracted}\n"
                except:
                    message_text += f"• User ID: `{user_id}`\n"
                    message_text += f"  └ Files Extracted: {user.get('files_extracted', 0)}\n"
        
        # Authorized users section
        message_text += "\n**✅ Authorized Users:**\n"
        for user_id in auth_users_list:
            try:
                user_info = await bot.get_users(user_id)
                username = user_info.username or "No Username"
                files_extracted = next((u.get("files_extracted", 0) for u in all_users if u["user_id"] == user_id), 0)
                message_text += f"• **{user_info.first_name}** (@{username})\n"
                message_text += f"  └ Files Extracted: {files_extracted}\n"
            except:
                message_text += f"• User ID: `{user_id}`\n"
                message_text += f"  └ Files Extracted: {next((u.get('files_extracted', 0) for u in all_users if u['user_id'] == user_id), 0)}\n"
        
        # Add total stats
        total_users = len(all_users)
        total_files = sum(user.get("files_extracted", 0) for user in all_users)
        message_text += f"\n**📈 Total Statistics:**\n"
        message_text += f"• Total Users: {total_users}\n"
        message_text += f"• Total Files Extracted: {total_files}\n"
        
        await message.reply(message_text)
    except Exception as e:
        logging.error(f"Error in show_stats: {e}")
        await message.reply("**Error fetching user statistics. Please try again later.**")

# Update help command to include new /stats command
@bot.on_message(filters.command(["help"]) & filters.user(auth_users[0]))
async def help_command(bot, message):
    help_text = (
        ...
        "👥 **User Management:**\n"
        "• `/auth <user_id>` - Authorize a user (permanent)\n"
        "• `/rem <user_id>` - Remove user authorization\n"
        "• `/users` - Show authorized users\n"
        "• `/stats` - Show detailed bot usage statistics\n\n"
        "⚙️ **Bot Settings:**\n"
        "• `/on` - Enable auth mode\n"
        "• `/off` - Disable auth mode\n\n"
        "**Note:** Authorized users have permanent access until manually removed."
    )
    await message.reply(help_text)

# Modify check_auth to include bot owner
async def check_auth(user_id: int) -> bool:
    try:
        # Bot owner always has auth
        if user_id == auth_users[0]:
            return True
        auth_user = await auth_collection.find_one({"user_id": user_id})
        return auth_user is not None
    except Exception as e:
...
# Modify get_remaining_quota to handle bot owner
async def get_remaining_quota(user_id: int) -> int:
    try:
        # Bot owner has unlimited quota
        if user_id == auth_users[0]:
            return float('inf')
